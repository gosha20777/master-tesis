...
class AWNetMini(nn.Module):
    def __init__(self, in_channels, out_channels, block=[2, 2, 2]):
        super().__init__()
        ...
        _layer_1_dw = []
        for i in range(block[0]):
            _layer_1_dw.append(GCRDB(32, ContextBlock2d))
        _layer_1_dw.append(GCWTResDown(32, ContextBlock2d, norm_layer=None))
        self.layer1 = nn.Sequential(*_layer_1_dw)

        # layer 2
        _layer_2_dw = []
        for i in range(block[1]):
            _layer_2_dw.append(GCRDB(64, ContextBlock2d))
        _layer_2_dw.append(GCWTResDown(64, ContextBlock2d, norm_layer=None))
        self.layer2 = nn.Sequential(*_layer_2_dw)

        # layer 3
        _layer_3_dw = []
        for i in range(block[2]):
            _layer_3_dw.append(GCRDB(128, ContextBlock2d))
        _layer_3_dw.append(GCWTResDown(128, ContextBlock2d, norm_layer=None))
        self.layer3 = nn.Sequential(*_layer_3_dw)
        ...
        self.sc_x1 = shortcutblock(32, 32)
        self.sc_x2 = shortcutblock(64, 64)

        self.scale_3 = nn.Conv2d(128, out_channels, kernel_size=3, padding=1)
        self.scale_2 = nn.Conv2d(64, out_channels, kernel_size=3, padding=1)
        self.scale_1 = nn.Conv2d(32, out_channels, kernel_size=3, padding=1)

        self.se1 = SE_net(32, 32)
        self.se2 = SE_net(64, 64)
        self.se3 = SE_net(128, 128)
    ...
    def forward(self, x, target=None, teacher_latent=None):
        ...
        x2, x2_dwt = self.layer1(self.se1(x1))
        x3, x3_dwt = self.layer2(self.se2(x2))
        x3_latent = self.layer3(self.se3(x3))

        x3_out = self.scale_3(x3_latent)
        x3_out = F.sigmoid(x3_out)
        x2_up = self.layer2_up(x3_latent, x3_dwt) + self.sc_x2(x2)
        x2_out = self.scale_2(x2_up)
        x2_out = F.sigmoid(x2_out)
        x1_up = self.layer1_up(x2_up, x2_dwt) + self.sc_x1(x1)
        x1_out = self.scale_1(x1_up)
        x1_out = F.sigmoid(x1_out)
        out = self.last(x1_up)
        return (out, x1_out, x2_out, x3_out), x3_latent
