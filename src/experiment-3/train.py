...
from losses.loss import ms_Loss
from utils.dataset import LoadData
from config import trainConfig
from utils.utils import validation, print_log, to_psnr, adjust_learning_rate_step
...
def train():
    ...
    new_lr = config.learning_rate[0]

    # Reload
    if config.pretrain == True:
        net.load_state_dict(
            torch.load(
                '{}/best_4channel.pkl'.format(config.weights_dir),
    ...
    train_dataset = LoadData(
        config.data_dir, TRAIN_SIZE, dslr_scale=1, test=False)
    ...
    test_dataset = LoadData(
        config.data_dir, TEST_SIZE, dslr_scale=1, test=True)
    ...
    for epoch in range(config.epoch):
        ...
        if epoch > 0:
            new_lr = adjust_learning_rate_step(
                optimizer, epoch, config.epoch, config.learning_rate)
        for batch_id, data in enumerate(train_loader):
            ...
            if config.print_loss:
                print("epoch:{}/{} | Loss: {:.4f} ".format(
                    epoch, config.epoch, total_loss.item()))
        ...
        torch.save(state, '{}/epoch_{}.pkl'.format(
            config.weights_dir, epoch))
        ...
        print_log(epoch + 1, config.epoch, one_epoch_time, train_psnr,
        ...
        if val_psnr >= pre_psnr:
            ...
            torch.save(state, '{}/best.pkl'.format(
                config.weights_dir))
            pre_psnr = val_psnr
